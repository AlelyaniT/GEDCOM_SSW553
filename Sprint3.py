...
class SolutionWithReport():
	...
	def generateReport(self):
		...
		for uid in sorted(list(self.people.keys())):
			...
			print "{}\t\t{}\t{}\t{}\t{}".format(uid, self.people[uid].name, self.people[uid].sex, self.people[uid].birth.strftime("%d %b %Y") if self.people[uid].birth is not None else "None", self.people[uid].death.strftime("%d %b %Y") if self.people[uid].death is not None else "None")
